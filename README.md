...
| **Levels** | 1–7. A difficulty the scenario has not authored is dimmed; selecting it starts one, with the scenario data of the nearest level it does author. Delete a level by deleting its notes: a difficulty with nothing to play is not one, so it leaves the ladder rather than becoming an error you have to clear. Its scenario data is remembered until you leave the editor, so putting a note back puts *that* level back. The scenario's last level cannot go — `loadScenario` needs one |
