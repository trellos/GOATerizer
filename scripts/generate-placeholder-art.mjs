#!/usr/bin/env node
/**
 * Generates Rocky Ascent's placeholder pixel art.
 *
 * ## Why generated rather than downloaded
 *
 * The scenario file names four third-party CC0 sources (karsiori's mountains
 * and rock piles, Spring Spring's goat strip and dust, CodeManu's effects pack).
 ...
 * ships today, and the file says so: the environment this slice was built in
 * cannot reach itch.io or opengameart.org, so the honest options were to ship
 * unverified art or to draw substitutes.
 *
...
const OUT_DIR = path.resolve(here, "..", "public", "assets", "scenarios", "rocky-ascent");
...
function goatPose(legs) {
  const image = new Pixels(24, 18);
  image.stamp(0, 0, GOAT_BODY, GOAT_PALETTE);
  image.stamp(0, GOAT_BODY.length, legs, GOAT_PALETTE);
  return image;
}

function goatFinish() {
  const image = new Pixels(24, 18);
  image.stamp(0, 0, GOAT_FINISH, GOAT_PALETTE);
  return image;
}
...
/** The summit cairn: the destination, visible from the start of the attempt. */
function goal() {
 ...
 *
 * Opaque, with the centre-right ascent corridor left uncluttered so a route of
 * up to thirty footholds stays readable on top of it.
 */
function background() {
  const W = 384;
  const H = 216;
  const image = new Pixels(W, H);
  const random = lcg(0x60a7);
...
const ASSETS = {
  bg_rocky_ascent: background,
  goat_rocky_ascent_advance_01: () => goatPose(GOAT_LEGS[0]),
  goat_rocky_ascent_advance_02: () => goatPose(GOAT_LEGS[1]),
  goat_rocky_ascent_advance_03: () => goatPose(GOAT_LEGS[2]),
  goat_rocky_ascent_advance_04: () => goatPose(GOAT_LEGS[3]),
  goat_rocky_ascent_finish: goatFinish,
  prop_rocky_ascent_step: step,
  prop_rocky_ascent_goal: goal,
  fx_rocky_ascent_dust: dust,
  fx_rocky_ascent_tick: tick,
};

mkdirSync(OUT_DIR, { recursive: true });
for (const [id, make] of Object.entries(ASSETS)) {
  const image = make();
  writeFileSync(path.join(OUT_DIR, `${id}.png`), image.toPng());
  console.log(`${id}.png  ${image.width}x${image.height}`);
}
console.log(`\nWrote ${Object.keys(ASSETS).length} placeholder assets to ${path.relative(process.cwd(), OUT_DIR)}`);
