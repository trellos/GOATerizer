...
try {
  ...
  //
  // The dense end is **L6**, not L4. The Scale content redo moved Rocky
  // Ascent's eighth-note material up the ladder and added L5-L7 (DECISION-049
  // re-pinned the unit tests to it and missed this suite): L4 is sixteen beats
  // of quarters like L1, so the comparison was measuring one rhythm against
  // itself and failing on both of its own assertions. L6 is the level that is
  // now thirty-two eighths.
  const DENSE_LEVEL = 6;
  const beatsToEightNotes = {};
  for (const [level, file] of [
    [1, "10-timeline-l1.png"],
    [DENSE_LEVEL, "11-timeline-l6.png"],
  ]) {
  ...
  note(
    `eight notes took ${beatsToEightNotes[1]} beats at L1, ` +
      `${beatsToEightNotes[DENSE_LEVEL]} at L${DENSE_LEVEL}`
  );
  check(
  ...
  check(
    `L${DENSE_LEVEL}'s eight eighth notes take about four`,
    beatsToEightNotes[DENSE_LEVEL] >= 3 && beatsToEightNotes[DENSE_LEVEL] <= 5.5,
    String(beatsToEightNotes[DENSE_LEVEL])
  );
  check(
    `L${DENSE_LEVEL} is denser than L1`,
    beatsToEightNotes[DENSE_LEVEL] < beatsToEightNotes[1],
    `${beatsToEightNotes[1]} → ${beatsToEightNotes[DENSE_LEVEL]} beats`
  );
