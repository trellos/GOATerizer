...
#### DECISION-025: A triplet is a duration name, and authored positions are derived on a tick grid
* **Date:** 2026-09-02
* **Status:** Accepted (resolves the content-model blocker recorded in DECISION-023)
* **Owner:** Trevor (agent-assisted)
* **Context:** GOATerizer could hear a triplet but not author one. `subdivisionsOf` already detects a triplet grid from note positions so the drums can mark it (GDD §10), and the design names Triplets as one of the six permanent families — but `NoteDuration` had no member for a third of a beat, `load.ts` required `durationBeats` to equal the table value exactly, and `TIMING_WINDOWS_BEATS` had no row to judge one against. `ThreeStepMinigame` was unbuildable, which DECISION-023 recorded as a blocker the minigame API deliberately did not solve. The obstacle is arithmetic, not design: 1/3 is not a binary fraction, so no decimal an author can type is exactly a third and no float sum of three of them is exactly a beat.
* **Decision:** Add `eighthTriplet` (1/3 beat) to `NoteDuration`, `DURATION_BEATS` and `TIMING_WINDOWS_BEATS` (`perfect: 0.08`, `good: 1/6`, interpolated between eighth and sixteenth rather than measured). In the loader, **the duration names become the authority**: length comes from the table and position from a running sum on a 12-tick-per-beat integer grid — twelve because it divides both the binary subdivisions and the triplet one — and the `durationBeats`/`startBeat` a file states are verified within `AUTHORING_TOLERANCE_BEATS` (0.01) and then discarded. `subdivisionsOf` also consults the new name, for a triplet whose other two thirds are rests and so leaves no position to read.
* **Alternatives Considered:** (a) Keeping authored `startBeat` as the runtime value with a loosened tolerance. Rejected: a written `0.333` is 0.00033 off, which is past `subdivisionsOf`'s 1e-6 epsilon, so the drums would stop marking the very grid the notes are on — and twelve of them miss the end of the phrase by 0.004 beats, failing the total-length check for a phrase that is musically exact. (b) Accumulating derived positions as floats. Rejected for the same reason at a smaller scale: three thirds sum to 0.9999999999999999, which puts a note a machine epsilon below beat 1 and, at the end of four measures, one measure lower than it belongs. (c) A `tuplet: {inSpaceOf}` modifier on each note instead of a duration name, which is what notation software does. Rejected as more expressive than anything on the roster needs, and it would put a second, differently-shaped length field in front of every minigame reading `Opportunity.duration`. (d) Dropping the authored `durationBeats`/`startBeat` fields now that both are derived. Rejected: `startBeat` is what catches a dropped rest, which is precisely the error a derived position absorbs silently.
* **Consequences:** Positive — the family is unblocked, and `tests/triplets.test.ts` carries a four-measure triplet phrase through load, grid detection and judgment, including that three notes a third apart do not steal each other's attacks. The tick grid also makes the *existing* content exact by construction rather than by luck: every prior duration was a binary fraction, so all 245 tests pass unchanged. Negative — the triplet timing row is a guess, flagged as such in `tuning.ts`, and it is the subdivision Tuninator is least accurate on (~94–124ms published onset error), so it is the row most likely to need retuning by ear. A duration that does not land on twelfths cannot be added without revisiting `TICKS_PER_BEAT`; a sixteenth triplet (2 ticks) fits, a quintuplet does not.

---

#### DECISION-024: The energy streak dies with the scenario panel; the delay becomes a tuning value
* **Date:** 2026-09-02
* **Status:** Accepted (supersedes the streak described in `GOATerizer_Game_Design.md` §6)
* **Owner:** Trevor (agent-assisted)
* **Context:** A judged note used to launch a short streak from the timeline into the scenario panel, and its **arrival** — about 0.28 beats later — was what called `deliverEnergy` and made the minigame react. That was deliberate: the player read "my note caused that" rather than "two things happened at once". The timeline is now the only surface (DECISION-023, GDD §11.2), so the judged note and the reaction are the same pixels and a streak between them has nowhere to fly. In collapsing the two surfaces the streak was deleted and the delay went with it, silently — an adversarial review of the shipped tree flagged that as a documented invariant reversed in passing rather than decided.
* **Decision:** The streak is gone; a zero-distance flight reads as a twitch, not as causation, and co-location is a stronger statement of cause and effect than a link between two regions ever was. The **delay** is separated from it and becomes `REACTION_DELAY_BEATS` in `config/tuning.ts`, defaulting to `0` — today's behaviour — owned by `AttemptRuntime` rather than by a visual effect, with a `reactionDelayBeats` test seam matching the pattern `JudgeOptions` already uses. At zero the note is handed over on the spot rather than on the next frame, so the default really is "the reaction happens when the note is judged". `EnergyLayer` survives for the one flight that still crosses regions: earned stars travelling from the timeline into the run history.
* **Alternatives Considered:** (a) Keeping the 0.28-beat delay as a constant. Rejected as a guess in the other direction: the delay's whole justification was the streak's flight time, and with the streak gone there is no principled value — only a feel question. (b) Deleting the delay outright and saying so in the GDD, which is what shipped. Rejected on review: it reverses a documented invariant, and "the goat steps on the same frame the note is judged" may read as crisp or as simultaneous-rather-than-caused, which is not decidable by argument. (c) Letting each minigame own its own reaction delay. Rejected: it is a property of how the *game* reads, not of one scenario, and six minigames each picking a different one is exactly the inconsistency the shared motion helpers exist to prevent. (d) Deleting `EnergyLayer` entirely. Rejected — the star flight genuinely crosses from the play surface into the top bar, and `update()` firing arrivals off the draw pass is what guarantees a star lands even if nobody watched it fly.
* **Consequences:** Positive — the reversal is explicit instead of incidental, and the alternative can now be tried by ear (`REACTION_DELAY_BEATS = 0.25`, play it, decide) rather than argued about; two tests pin both sides of the behaviour. Negative — a tuning knob that nobody has yet tuned is a decision deferred, not made, and it will stay at zero by inertia unless someone deliberately plays the other value. `EnergyLayer` also now carries a `bad` polarity and colour with no producer; that is one line kept on the judgement that a minigame reaction throwing something across the screen will want it back.
